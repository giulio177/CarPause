...
- `backend.py`: High-performance QObject bridge exposing reactive `Q_PROPERTY` attributes and non-blocking `@pyqtSlot` handlers to QML.
  ...
  - Wi-Fi management (`wifiPowered`, `wifiConnected`, `wifiSsid`, `wifiSignal`, `availableWifiNetworks`, `wifiConnecting`, `wifiScanning`), connection/disconnection slots, and active probe rescan (`rescanWifi`).
...
- `src/services/`: Linux OS and hardware services (zero Qt dependency):
  ...
  - `network_service.py`: Real NetworkManager (`nmcli`) Wi-Fi scanner with active 802.11 probe rescan support (`--rescan yes`), active IP address detection, and BlueZ (`bluetoothctl`) tracking with discoverable broadcast and active device scanning.
