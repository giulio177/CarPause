...
- `backend.py`: High-performance QObject bridge exposing reactive `Q_PROPERTY` attributes and non-blocking `@pyqtSlot` handlers to QML.
  ...
  - OpenStreetMap management: reactive properties (`mapCenterLat`, `mapCenterLon`, `mapZoom`, `mapTheme`, `mapTileBaseUrl`, `mapSearchResults`, `mapSearching`, `mapBookmarks`) and non-blocking slots (`setMapCenter`, `panMap`, `setMapZoom`, `zoomIn`, `zoomOut`, `setMapTheme`, `cycleMapTheme`, `locateVehicle`, `searchMap`, `clearMapSearch`, `addMapBookmark`, `removeMapBookmark`). Auto-locates vehicle via hotspot IP geolocation on screen navigation.
...
- `src/services/`: Linux OS and hardware services (zero Qt dependency):
  - `map_service.py`: OpenStreetMap service running a local multi-threaded HTTP tile proxy (127.0.0.1:8585) with persistent disk caching in `~/.cache/mito_maps/{theme}/{z}/{x}/{y}.png` supporting CartoDB Dark No-Labels (`dark_clean`), CartoDB Dark Matter (`dark`), CartoDB Voyager (`voyager`), and OpenStreetMap Standard (`osm`), IP geolocation (`get_ip_location`) for phone hotspot position tracking, asynchronous Nominatim geocoding place search, Slippy Map tile projection calculations, and bookmarks management.
