...
- `qml/`: Pure declarative UI, modular automotive architecture fixed at 1024x600 resolution:
  ...
  - `bars/`: `TopBar.qml` (borderless status bar seamlessly blending into background with dynamic Material Symbols for weather conditions, clock, CPU, battery, and tappable Wi-Fi / Bluetooth status pills with generous touch margins opening their respective settings subpages), `BottomBar.qml` (compact mini-media player, screen navigation tabs with centered icon and small text underneath: DASH, MEDIA, AIRPLAY, SETTING using native `TapHandler` with touch margins and inner content scaling to prevent touch-target shrinkage, app restart / 3-second hold reboot button, and volume slider dynamically bounded by `maxVolume` with amber boost indicator for volumes > 100%).
