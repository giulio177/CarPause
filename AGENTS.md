...
- `src/services/`: Linux OS and hardware services (zero Qt dependency):
  - `airplay_service.py`: Subprocess lifecycle manager for UxPlay AirPlay mirroring daemon (supports default Broadcom GPU hardware `-bt709` 60 FPS decoding and fallback software `-avdec` FFmpeg NEON decoding, direct DRM framebuffer `-vs kmssink` rendering, unbuffered line I/O via `stdbuf`, automatic process teardown and restart on client disconnect to prevent frozen last-frames, touch tap event notification to reveal floating exit X button without interrupting streaming, and 5-second continuous touch hold watchdog triggering `killall -9 uxplay`).
