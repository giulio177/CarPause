...
- `src/services/`: Linux OS and hardware services (zero Qt dependency):
  ...
  - `network_service.py`: Real NetworkManager (`nmcli`) Wi-Fi scanner with active 802.11 probe rescan support (`--rescan yes`), active IP address detection, automatic PolicyKit permission fallback handling (`_run_nmcli_command`), and BlueZ (`bluetoothctl`) tracking with discoverable broadcast and active device scanning.
